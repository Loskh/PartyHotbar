...
internal static unsafe class NodeLinker
{
    ...
    internal static void AttachNode(AtkResNode* node, AtkComponentNode* attachTargetComponentNode, NodePosition position)
    {
        var attachTargetNode = attachTargetComponentNode->Component->UldManager.RootNode;
        switch (position)
        {
            case NodePosition.BeforeAllSiblings:
                EmplaceBeforeSiblings(node, attachTargetNode);
    ...
    public static void DetachNode(AtkResNode* node, bool free = true)
    {
        if (node is null) return;
        if (node->ParentNode is null) return;
        Service.PluginLog.Debug($"Detaching {(nint)node:X}:{node->NodeId} {node->GetNodeType()}");
    ...
    public static AtkUldManager* GetUldManagerForNode(AtkResNode* node)
    {
    ...
    public static void VisitChildren(AtkResNode* parent, Action<Pointer<AtkResNode>> visitAction)
    {
        visitAction(parent);
